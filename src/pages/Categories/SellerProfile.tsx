...
//   "https://tradelink-be.onrender.com/api/v1/reviews",
//   {
//     sellerId,
//     userId,
//     rating: reviewRating,
...
const ReviewsPopup = ({ reviews }: { reviews: Review[] }) => {
  ...
  return (
    <div className="fixed bottom-24 right-6 z-50">
      {open ? (
        <div className="w-96 h-[28rem] bg-white shadow-xl rounded-xl border flex flex-col">
          {/* Header */}
          <div className="flex justify-between items-center p-3 border-b bg-orange-500 text-white rounded-t-xl">
            <h3 className="font-semibold">Seller Reviews</h3>
            <button
        ...
        <button
          ...
          className="bg-orange-500 hover:bg-orange-600 text-white p-4 rounded-full shadow-lg flex items-center gap-2"
        >
          <MessageCircle className="w-6 h-6" />
  ...
  useEffect(() => {
    const fetchSellerProfile = async () => {
      try {
        ...
        setProducts(res.data?.products || []);
        setStats(res.data?.stats || null);
        setReviews(res.data?.reviews || []); // ✅ reviews come from here
      } catch (err) {
        console.error("Error fetching seller profile:", err);
      } finally {
        setLoading(false);
